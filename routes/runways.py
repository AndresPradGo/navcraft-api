...
@router.post("/runway", status_code=status.HTTP_201_CREATED, response_model=schemas.RunwayReturn)
async def post_runway_(
    ...
    private_waypoint = db.query(models.UserWaypoint).filter_by(
        waypoint_id=runway_data.aerodrome_id).first()
    if not aerodrome_is_registered and not private_waypoint.creator_id == user_id:
        raise no_permission_exception
