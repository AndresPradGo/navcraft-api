...
> NOTE: The original database design is shown inside the red box in the conceptual diagram. The Weather group was added to it afterwards, as part of the design of the [NavBrix API](https://navbrixapi.com/docs); an API that adds weather briefing capabilities to the NavCraft API.
...
> NOTE: The `/src` directory in the host, is being mapped to the `/src` directory in the container. Thus, any changes you save will be automatically shared to the container. However, the `pipfile` and `pipfile.lock` files are not being mapped. If you install a new library, you'll need to rebuild the image for it to show in the container.
